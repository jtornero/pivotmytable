...
as_percentage(bool):    This option enables the output as percentage of each output
                        columns over its overall sum BY ROW. It defaults to FALSE, so
                        to enable percentage calculation set it to TRUE.
...
AS $$
    ...
    inputTableColumnsQuery=plpy.execute("select column_name from information_schema.columns where table_name='%s'" %input_table)
    inputTableColumnsList = [col["column_name"] for col in inputTableColumnsQuery]
    
    if input_table not in tableList:
        return "Your input table %s does not exist.  Please check and try again." %input_table
        
    for parameter in group_fields.split(','):
        if parameter not in inputTableColumnsList:
    ...
    if pivot_field not in inputTableColumnsList:
    ...
    if value_field not in inputTableColumnsList:
    ...
    if (output_table in tableList):
        if (drop_ex_tbl):
            plpy.execute("drop table %s" %output_table)
        else:
            return "The table %s already exists in the database.  Please check and try again." %output_table
    ...
    columnPropertiesQuery = plpy.execute("select data_type,numeric_scale from information_schema.columns where table_name = '%s' and column_name='%s'" %(input_table,value_field))
    ...
    gfields=group_fields.split(',')
    groupingColumns=list()
    idx=1
    for field in gfields:
        columnType = plpy.execute("select data_type from information_schema.columns where table_name = '%s' and column_name='%s'" %(input_table,field))
    ...
    finalQuery = ("create table %s as (select %s,%s from crosstab('%s','%s') as newtable(%s))" %(output_table,groupingColumns,replaze0,firstQuery,secondQuery,crossTabQuery))  
    ...
    return "Your pivoted table %s has been created." %(output_table)
